...
#k means cluster as engineering feature and logistic regression
from sklearn.linear_model import LogisticRegression
X_basis = data[float_columns]
sss = StratifiedShuffleSplit(n_splits=10, random_state=6532)

def creat_kmeans_columns(n):
    km = KMeans(n_clusters=n)
    km.fit(X_basis)
    km_col = pd.Series(km.predict(X_basis))
    km_cols = pd.get_dummies(data = km_col, prefix= 'kmeans_cluster')
    return pd.concat([X_basis, km_cols], axis = 1)

estimator = LogisticRegression()
ns = range(1,21)
roc_auc_list1 = get_avg_roc_10splits(estimator, X_without_kmeans , y)
roc_auc_list2 = [get_avg_roc_10splits(estimator, creat_kmeans_columns(n), y) for n in ns]
#plot
ax = plt.axes()
